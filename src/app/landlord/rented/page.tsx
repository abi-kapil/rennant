...
export default function LandlordRentedPage() {
  ...
  const rentedProperties = properties.filter((p) =>
    landlord.rentedPropertyIds.includes(p.id)
  );

  const cities = useMemo(
    () => [...new Set(rentedProperties.map((p) => p.city))].sort(),
    [rentedProperties]
  );

  const filtered = useMemo(() => {
    let result = [...rentedProperties];
    ...
    return result;
  }, [rentedProperties, searchQuery, sortBy, bedsFilter, cityFilter]);
